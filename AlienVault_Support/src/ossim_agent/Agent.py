...
from Conn import ServerConn, IDMConn, FrameworkConn, LoggerConn
...
class Agent:
    ...
    def __init__( self ):
        ...
        if self.options.config_file:
            ...
            self.__conffile = Defaults.DEFAULT_CONFIG_FILE
    ...
    def __getSensorID(self):
        sensor_id =""
        system_file = Defaults.DEFAULT_SYSTEM_FILE
        ...
        if self.conf.has_option("plugin-defaults","system_uuid_file"):
            ...
            logger.warning("system uuid file not configured, using default %s" % Defaults.DEFAULT_SYSTEM_FILE)
    ...
    def connect_framework( self ):
        ...
        if not self.__outputServerConnection.allow_frmk_data:
            return
...
#                    self.detector_objs.append( parser )
    ...
    def __readLoggerServer(self):
        if self.conf.has_section(Defaults.STRINGS_LOGGER_SECTION):
            if self.conf.getboolean(Defaults.STRINGS_LOGGER_SECTION, Defaults.STRINGS_ENABLE):
                server_ip = self.conf.get(Defaults.STRINGS_LOGGER_SECTION, Defaults.STRINGS_IP)
                server_port = self.conf.get(Defaults.STRINGS_LOGGER_SECTION, Defaults.STRINGS_PORT)
                self.__loggerConnection = LoggerConn(server_ip, server_port)
                Output.add_logger_output(self.__loggerConnection)
    ...
    def __readOuptutServer( self ):
        ...
        if self.conf.has_section( "output-server" ):
            if self.conf.getboolean( "output-server", "enable" ):
                ...
                if self.conf.has_option("output-server", "request_framework"):
                    allow_frmk_data = self.conf.getboolean("output-server", "request_framework")
    ...
    def __check_server_status( self ):
        ...
        while self.__keep_working:
            ...
            if not self.__outputServerConnection.get_is_alive():
