...
class LoggerConn:
    MSG_CONNECT = "Hello I am a OSSIM Agent"
    __conn = None
    ...
    def connect(self, attempts=3, waittime=10.0):
        ...
        if self.__connection is None:
            logger.info("Connecting to logger (%s, %s).." \
                        % (self.server_ip, self.server_port))

            while attempts >= count and not self.__connect():
                logger.info("Can't connect to logger, retrying in %d seconds" % (waittime))
                time.sleep(waittime)
                count += 1

        else:
            logger.info("Reusing logger connection (%s, %s).." \
                        % (self.server_ip, self.server_port))
    ...
    def close(self):
        if self.__connection is not None:
            logger.info("Closing logger connection..")
            try:
                self.__connection.shutdown(socket.SHUT_RDWR)
                self.__connection.close()

            except Exception,e:
                logger.info("Failed to close connection: " + self.server_ip + " cleanly...")
                pass
    ...
    def __connect(self):
        ...
        try:
            self.__connection.connect((self.server_ip, int(self.server_port)))
            self.__connection.send(self.MSG_CONNECT)
            logger.debug("Waiting for logger..")
            ...
            if data == 'HELLO\n':
                logger.info("Connected to logger %s:%s!" % (self.server_ip, self.server_port))
                self.__closed = False
            else:
                logger.error("Bad response from logger: %s " % (str(data)))
                try:
                    self.__connection.shutdown(socket.SHUT_RDWR)
                    self.__connection.close()
                except:
