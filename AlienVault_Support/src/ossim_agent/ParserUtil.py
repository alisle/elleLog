...
GEOIPDB=GeoIP.open(Defaults.DEFAULT_GEOIP,GeoIP.GEOIP_STANDARD)
#
# LOCAL IMPORTS
#
...
class HostResolv():
                    ...
                    for hostname in hostname_list:
                        ...
                        if re.match(ipv4_reg, ip) and re.match(hostname_valid, hostname):
                            if HostResolv.HOST_RESOLV_DYNAMIC_CACHE.has_key(hostname):
                                if ip not in HostResolv.HOST_RESOLV_DYNAMIC_CACHE[hostname]:
                                    HostResolv.HOST_RESOLV_DYNAMIC_CACHE[hostname].append(ip)
                            else:
                                HostResolv.HOST_RESOLV_DYNAMIC_CACHE[hostname] = []
                                HostResolv.HOST_RESOLV_DYNAMIC_CACHE[hostname].append(ip)
    ...
    def saveHostCache():
        logger.info("Saving dynamic host cache in " + Defaults.DEFAULT_HOST_CACHE )
        pickle.dump(HostResolv.HOST_RESOLV_DYNAMIC_CACHE, open(Defaults.DEFAULT_HOST_CACHE, "wb"))
    saveHostCache = staticmethod(saveHostCache)

    def loadHostCache():
        if os.path.isfile(Defaults.DEFAULT_HOST_CACHE):
            try:
                logger.debug("Loading dynamic host cache from '" + Defaults.DEFAULT_HOST_CACHE + "'")
                HostResolv.HOST_RESOLV_DYNAMIC_CACHE = pickle.load(open(Defaults.DEFAULT_HOST_CACHE))
                HostResolv.printCache()
            except:
                # Why are these called something else? Artefact from the Pro Version.
                #logger.warning("Deleting corrupt file host_cache_pro.dic")
                #os.remove("/etc/ossim/agent/host_cache_pro.dic")
                logger.warning("Deleting corrupt file " + Defaults.DEFAULT_HOST_CACHE)
                os.remove(Defaults.DEFAULT_HOST_CACHE)
                return False
        else:
            return False
